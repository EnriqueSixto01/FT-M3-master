...
function problemA() {
  ...
  // promise version
  // ???
  let stanzas = ["poem-two/stanza-01.txt", "poem-two/stanza-02.txt"];
  var files = stanzas.map((el) => promisifiedReadFile(el));
...
function problemB() {
  ...
  // promise version
  // ???
  Promise.all(filenames.map((file) => promisifiedReadFile(file))).then(
    (data) => {
      data.map((e) => blue(e));
      console.log("done");
    }
